...
def add_sell():
    ...
    item = Item(
        ...
        sold=False, time=datetime.now().strftime("%Y-%m-%d %H:%M"), views=0
    )
    db.session.add(item)
    db.session.commit()
    return redirect('/')
...
def add_buy():
    ...
    item = Item(
        ...
        time=datetime.now().strftime("%Y-%m-%d %H:%M"), views=0
    )
    db.session.add(item)
    db.session.commit()
    return redirect('/')
...
def sold(item_id):
    item = Item.query.get(item_id)
    student_id = request.form.get('student_id', '').strip()
    if item and item.type == "sell" and item.student_id == student_id:
        item.sold = True
        db.session.commit()
...
def delete(item_id):
    item = Item.query.get(item_id)
    student_id = request.form.get('student_id', '').strip()
    if item and item.student_id == student_id:
